...
const handleSubmit=async(e:any)=>{
    ...
    try {
        if(password.value.length<7){
            ...
            let url=`${origin}/api/auth/group/login`
...
<template>
    <div class="flex flex-col bg-[#fffbf7] justify-center items-center h-[100vh]">
        <div class="flex flex-col justify-center items-center md:w-[350px] max-md:w-[80vw] ">
            <p class="text-2xl font-semibold mb-4 text-gray-700">Join your <span class="text-green-400">Group</span></p>
            <form class="my-3 flex flex-col w-full" @submit="handleSubmit" @keydown="checkInput" >
                <label for="email" class="ml-1 flex justify-between max-md:text-sm"><span>Enter your organization email</span></label>
            ...
            <div class="flex justify-between items-center gap-10 mb-10 w-full text-sm">
                <div @click="router.push('/choose')" class="text-green-400 font-semibold  cursor-pointer">Create an account?</div>
                <div @click="router.push('/signin')" class="text-green-400 font-semibold cursor-pointer">sign in to your account?</div>
            </div>
        </div>
    </div>
</template>
